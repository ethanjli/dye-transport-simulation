...
void solvePoisson(Grid &x, const Grid &x_0, Scalar a, Scalar c,
    ...
    Grid temp;
    Grid::Index N = x_0.rows();

    x = x_0;
    for (unsigned int iteration = 0; iteration < numIterations; ++iteration) {
        for (Grid::Index i = 1; i <= N; ++i) {
            for (Grid::Index j = 1; i <= N; ++j) {
...
void diffuseField(Grid &x, const Grid &x_0, Scalar diff,
    ...
    Scalar a = dt * diff * x_0.rows() * x_0.cols();
    solvePoisson(x, x_0, a, 1 + 4 * a, setBoundaries);
}

void advectField(Grid &newField, const Grid &field, const Grid &u, const Grid &v,
                 std::function<void(Grid&)> setBoundaries, Scalar dt) {
    Grid::Index N = field.rows();
    Scalar dt_0 = dt * N;
    for (Grid::Index i = 1; i <= field.rows(); ++i) {
        for(Grid::Index j = 1; j <= field.cols(); ++j) {
            Scalar x = i - dt_0 * u(i, j);
            x = std::max(0.5, std::min(N + 0.5, x));
            int i_0 = (int)x;
            int i_1 = i_0 + 1;
            Scalar s_1 = x - i_0;
            Scalar s_0 = 1 - s_1;
            Scalar y = j - dt_0 * v(i, j);
            y = std::max(0.5, std::min(N + 0.5, y));
            int j_0 = (int)y;
            int j_1 = j_0 + 1;
            Scalar t_1 = y - j_0;
            Scalar t_0 = 1 - t_1;
...
void projectField(Grid &u, Grid &v, Grid &p, Grid &div) {
    Grid::Index N = v.rows();
    p = Grid::Zero();
    for (Grid::Index i = 1; i <= v.rows(); ++i) {
        for(Grid::Index j = 1; j <= v.cols(); ++j) {
...
void setFieldBoundaries(Grid &grid, int b) {
    Grid::Index N = grid.rows();
    for (Grid::Index i = 1; i <= grid.rows(); ++i) {
        grid(i, 0) = (b == 2 ? -1 : 1) * grid(i, 1);
        grid(i, N + 1) = (b == 2 ? -1 : 1) * grid(i, N);
    }
    N = grid.cols();
    for(Grid::Index j = 1; j <= grid.cols(); ++j) {
        grid(0, j) = (b == 1 ? -1 : 1) * grid(1, j);
        grid(N + 1, j) = (b == 1 ? -1 : 1) * grid(N, j);
    }
}
